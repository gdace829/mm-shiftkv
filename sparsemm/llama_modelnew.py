...
def llama_flash_attn2_forward_AdaKV(
    ...
    if is_prefill:
        ...
        # We append 1 token per KV head at decode step. In GQA, num_kv_heads = num_heads // num_key_value_groups.
        # Using num_heads here overcounts and corrupts cu_klen/klen_sum, which later causes OOB in varlen kernels.
        self.kv_cluster.klen_sum += (self.num_heads // self.num_key_value_groups)
        self.kv_cluster.max_seqlen_k += 1
        self.kv_cluster.cu_klen += self.kv_cluster.cu_offset
        self.kv_cluster.head_lens += 1

        # Reshape for varlen FlashAttention: treat each KV head as a separate sequence and group queries per KV group
        n_kv = self.num_heads // self.num_key_value_groups
        query_states = query_states.view(bsz * n_kv, self.num_key_value_groups, self.head_dim)
        ...
        # Sanity checks to avoid illegal memory accesses in varlen kernels
        assert query_states.shape[0] == cu_seqlens_q[-1].item(), (
            f"Q batch mismatch: got {query_states.shape[0]}, expected {cu_seqlens_q[-1].item()}"
        )
        assert key_states.shape[0] == cu_seqlens_k[-1].item(), (
            f"K batch mismatch: got {key_states.shape[0]}, expected {cu_seqlens_k[-1].item()}"
        )
